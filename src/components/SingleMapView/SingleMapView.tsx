...
interface SingleMapViewProps {
  ...
  onSnapshot?: (mapContainer: HTMLElement, mapId: string) => void;
}

// Handles map size invalidations when the container size changes
...
export const SingleMapView: React.FC<SingleMapViewProps> = memo(
  ({
    ...
    useEffect(() => {
      localMapRef.current?.invalidateSize();
    }, [mapConfigsLength]);
    ...
    return (
      <div
        className={`${styles["single-map-view"]} ${
          isPrimary ? styles["primary-map"] : ""
        }`}
      >
        ...
        >
          {onSnapshot && (
            <button
              onClick={() => {
                const container = localMapRef.current?.getContainer();
                if (container) {
                  onSnapshot(container, config.id);
                } else {
                  console.warn("Snapshot failed: map container not available.");
                }
              }}
              style={{
                position: "absolute",
