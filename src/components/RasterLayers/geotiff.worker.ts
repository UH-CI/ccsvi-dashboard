...
addEventListener("message", async ({ data }) => {
  ...
  try {
    const tiff: geotiff.GeoTIFF = await geotiff.fromArrayBuffer(buffer);
    ...
    if (bbox && bbox.length >= 4) {
      ...
      Array.isArray(fileDirectory.ModelPixelScale) &&
      fileDirectory.ModelPixelScale.length >= 2
    ) {
      ...
      try {
        ...
        if (mainBbox && mainBbox.length >= 4) {
          ...
          if (
            mainFileDirectory.ModelPixelScale &&
            Array.isArray(mainFileDirectory.ModelPixelScale) &&
            mainFileDirectory.ModelPixelScale.length >= 2
          ) {
        ...
        throw new Error(
          `Cannot determine pixel scale and origin: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`,
        );
      }
    }
    ...
    const bandData: Record<number, [number, number][]> = {};
      ...
      try {
        ...
        if (totalPixels > 10000000) {
          ...
          for (let yStart = 0; yStart < height; yStart += windowHeight) {
            ...
            try {
              const windowRasters = (await image.readRasters({
                window,
                samples: [band],
              })) as geotiff.TypedArray[];
              ...
              if (windowRaster) {
                ...
                for (let j = 0; j < windowLength; j += sample) {
                  ...
                  if (values.length >= MAX_VALUES_PER_BAND) {
                    console.warn(
                      `Reached maximum values limit (${MAX_VALUES_PER_BAND}) for band ${band}, stopping processing`,
                    );
                    break;
                  }
        ...
        throw new Error(
          `Failed to read raster data: ${readError instanceof Error ? readError.message : String(readError)}`,
        );
      }
