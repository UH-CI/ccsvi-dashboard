...
export class ColorGeneratorService {
  ...
  getDefaultRainbowRainfallColorScale(dataRange: [number, number], reverse: boolean): ColorScale {
    const parts = ColorGeneratorService.COLOR_PARTS;
    const range: [number, number] = dataRange;
  ...
  async getColorSchemeFromAssetFile(
    xmlFile: string,
    dataRange: [number, number],
    reverse: boolean = false,
  ): Promise<XMLColorSchemeData> {
        ...
        if (err) {
          ...
          try {
            ...
            for (const point of points) {
              ...
              x = x * span + range[0];
              const r = parseFloat(point.$.r) * 255;
              const g = parseFloat(point.$.g) * 255;
              const b = parseFloat(point.$.b) * 255;
              const color = [r, g, b];
              colors.push(color);
              domain.push(x);
            }
