...
export class ColorGeneratorService {
  static readonly COLOR_PARTS = 500;
  ...
  getDefaultMonochromaticRainfallColorScale(dataRange: [number, number], reverse: boolean): ColorScale {
  ...
  async getColorSchemeFromAssetFile(xmlFile: string, dataRange: [number, number], reverse: boolean = false): Promise<XMLColorSchemeData> {
    const response = await fetch(xmlFile);
    if (!response.ok) {
      throw new Error(`Failed to fetch color scheme file: ${response.statusText}`);
    }
    const data = await response.text();
    return this.getColorSchemeFromXML(data, dataRange, reverse);
  }

  getColorSchemeFromXML(xmlData: string, dataRange: [number, number], reverse: boolean = false): Promise<XMLColorSchemeData> {
      ...
      parser.parseString(xmlData, (err: any, result : any) => {
        ...
        if(err) {
          ...
          try {
            ...
            let colorScaleF = chroma.scale(colors).domain(domain as any);
  ...
  getTurboColorScale(dataRange: [number, number], reverse: boolean): ColorScale {
    let parts = ColorGeneratorService.COLOR_PARTS;
    let range: [number, number] = dataRange;
    let colors = [
      ...
      [0.99163,0.68408,0.20706],[0.99314,0.67408,0.20348],[0.99438,0.66386,0.19971],[0.99535,0.65341,0.19577],[0.99607,0.64277,0.19165],
      ...
      [0.80799,0.17753,0.01660],[0.79971,0.17055,0.01520],[0.79125,0.16368,0.01387],[0.78260,0.15693,0.01264],[0.77377,0.15028,0.01148],
    ...
    //temp? should this be handled differently?
    //note this one is already reversed
    if(!reverse) {
      colors = colors.reverse();
    }
  ...
  getUSGSColorScale(dataRange: [number, number], reverse: boolean): ColorScale {
    let parts = ColorGeneratorService.COLOR_PARTS;
    let range: [number, number] = dataRange;
    ...
    [0.047,0.184,0.477], [0.48,0.188,0.566]];
    if(reverse) {
      colors = colors.reverse();
    }
  ...
  getViridisColorScale(dataRange: [number, number], reverse: boolean) {
    let parts = ColorGeneratorService.COLOR_PARTS;
    let range: [number, number] = dataRange;

    let colors = [[253, 231, 37], [180, 222, 44], [109, 205, 89], [53, 183, 121], [31, 158, 137], [38, 130, 142], [49, 104, 142], [62, 74, 137], [72, 40, 120], [68, 1, 84]];
    if(reverse) {
      colors = colors.reverse();
    }
    let colorScale = chroma.scale(colors).domain(range).correctLightness();
