          ...
          if(header === null) {
            ...
            for(let band in bandData) {
              ...
              for (let i = 0; i < Math.min(bandArray.length, maxEntries); i += batchSize) {
                const batch = bandArray.slice(i, Math.min(i + batchSize, bandArray.length, maxEntries));
                ...
                if (values.size >= maxEntries) {
                  console.warn(`Map size limit reached for band ${band}, stopping at ${values.size} entries`);
                  break;
                }
              }
