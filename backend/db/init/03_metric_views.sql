...
SELECT
    ...
    MAX(mv.percentage) FILTER (WHERE m.dataset_id = 'person_under_5_65_total'
        AND m.name = 'Total Under 5 (calc.)') AS pop_under_5_pct_calc,
    MAX(mv.percentage) FILTER (WHERE m.dataset_id = 'person_under_5_65_total'
        AND m.name = 'Total Under 18 (calc.)') AS pop_under_18_pct_calc,
    MAX(mv.percentage) FILTER (WHERE m.dataset_id = 'person_under_5_65_total'
        AND m.name = 'Total Over 65 (calc.)') AS pop_over_65_pct_calc,
