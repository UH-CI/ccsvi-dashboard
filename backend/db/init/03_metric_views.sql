...
SELECT
    ...
    MAX(mv.absolute) FILTER (WHERE m.dataset_id = 'population_group_quarters'
        AND m.name = 'Total: Institutionalized population') AS group_quarters_total_abs,
    MAX(mv.absolute) FILTER (WHERE m.dataset_id = 'population_group_quarters'
        AND m.name = 'Total: Institutionalized population: Correctional facilities for adults') AS group_quarters_correctional_abs,
    MAX(mv.absolute) FILTER (WHERE m.dataset_id = 'population_group_quarters'
        AND m.name = 'Total: Institutionalized population: Juvenile facilities') AS group_quarters_juvenile_abs,
    MAX(mv.absolute) FILTER (WHERE m.dataset_id = 'population_group_quarters'
        AND m.name = 'Total: Institutionalized population: Nursing facilities/Skilled-nursing facilities') AS group_quarters_nursing_abs,
    MAX(mv.absolute) FILTER (WHERE m.dataset_id = 'population_group_quarters'
        AND m.name = 'Total: Institutionalized population: Other institutional facilities') AS group_quarters_other_abs,
