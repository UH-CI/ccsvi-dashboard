...
This built census_metrics_by_block_group.json, which fed backend/ingest/load_metrics.py
and the old census_metrics table. That table was migrated to the current normalized
schema (datasets/metrics/metric_values/geographies) by backend/ingest/migrate_schema.py,
which then dropped census_metrics — see the DEPRECATED notes in
backend/app/routers/block_groups.py. Nothing reads census_metrics_by_block_group.json
anymore. The current path for loading cleaned data is a one-off script that upserts
directly into datasets/metrics/metric_values — see ADDING_DATASETS_GUIDE.md §2.

Left here commented out, not deleted, purely as a reference for the JSON shape this
used to produce.
"""
